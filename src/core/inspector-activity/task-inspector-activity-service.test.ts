...
it('counts only observed seeding intervals across pauses and disconnects', () => {
  const ctx = setup()
  const sample = (status: TaskStatus) =>
    ctx.service.recordSamples([ctx.sample({ status })])
  sample(TaskStatus.Downloading)
  ctx.advance(1000)
  sample(TaskStatus.Downloading)
  sample(TaskStatus.Seeding)
  ctx.advance(1000)
  sample(TaskStatus.Seeding)
  sample(TaskStatus.Paused)
  ctx.advance(1000)
  sample(TaskStatus.Paused)
  sample(TaskStatus.Seeding)
  ctx.advance(1000)
  ...
  sample(TaskStatus.Seeding)
  ctx.service.forceCheckpoint('task-1')
  const recorded = ctx.store.batches
    .flat()
    .reduce((sum, checkpoint) => sum + (checkpoint.seedingMsDelta ?? 0), 0)
