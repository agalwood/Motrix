...
const persistedEngineSettingsSchema = z.object({
  ...
  maxConcurrentDownloads: z.number().int().min(1).max(100).catch(10),
  ...
  minSplitSize: z
    ...
    .min(1048576)
    .max(1073741824)
    .catch(AUTO_PERFORMANCE.minSplitSize),
  ...
  sqlite3DbPath: z
    ...
    .refine((p) => !p.includes('\0'), {
      params: { settingIssue: 'text' },
    })
    .catch(''),
  sqlite3HistoryLimit: z.number().int().min(-1).max(1_000_000).catch(-1),

  // Magnet (motrix-turbo timer)
  magnetResolveTimeout: z.number().int().min(30).max(900).catch(600),
})

export const engineSettingsInputSchema = settingsInputObject(
  persistedEngineSettingsSchema
)

export const engineSettingsSchema = persistedEngineSettingsSchema.transform(
  applyEnginePerformanceProfile
)
