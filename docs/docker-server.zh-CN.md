...
窄屏上改为分组位置菜单。到达当前允许根目录后不能继续向上。打开“显示隐藏文件夹”
可显示名称以点开头的文件夹。
...
通过 **设置 → 下载 → 管理目录** 可添加或移除常用目录、将最近目录加入常用、逐条删除
最近记录或清空最近记录。管理动作立即保存，点击“关闭”返回上一个窗口；移除记录不会
删除文件夹或下载任务。常用目录最多20个，最近目录最多10个，最近使用的排在前面。
普通目录字段确认选择、AddTask成功提交时使用的目录都会更新最近记录。取消选择器不
...
可访问的目录；管理窗口保留已保存的记录，便于移除不可用路径。Web选择器的星按钮可
将当前正在浏览的目录加入常用或移除。上述操作都不改变默认保存目录。
