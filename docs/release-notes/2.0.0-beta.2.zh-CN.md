...
[English](https://github.com/agalwood/Motrix/blob/v2.0.0-beta.2/docs/release-notes/2.0.0-beta.2.md) | 简体中文

Motrix 2.0.0-beta.2 是首个计划对外发布的 Motrix Turbo beta。
该版本开始公开验证重新开发的 v2 桌面应用与 Headless server、共用下载
内核、MDXP 集成和沙箱化插件系统。

`v2.0.0-beta.1` tag 触发了受保护的发布路径，但两条由 tag 触发的
流水线都在对外分发前停止。beta.1 未创建 GitHub Release，也未发布 R2
更新数据源、Docker Hub 或 GHCR 容器镜像及 Snap Store revision。本 beta.2
版本取代 beta.1。
...
| Docker Hub / GHCR | `linux/amd64`、`linux/arm64` | 两个 registry 中均发布不可变 tag `2.0.0-beta.2` |
| Snap Store | `amd64`、`arm64` | `latest/edge` |

容器镜像地址为 `docker.io/motrixapp/motrix-server:2.0.0-beta.2` 和
`ghcr.io/agalwood/motrix-server:2.0.0-beta.2`。数据持久化、网络和升级方法请参阅
[Docker Server 部署指南](https://github.com/agalwood/Motrix/blob/v2.0.0-beta.2/docs/docker-server.zh-CN.md)。
